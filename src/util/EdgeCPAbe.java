...
//            Element c4x=gYi.powZn(rx);
            ...
            Element gALambda=pairing.getG1().newElement();
            gALambda.setFromBytes(GP.getGa());
            ct.setgALambda(gALambda.powZn(lambdaX).toBytes());
        }
...
//        Userkeys userkeys =new Userkeys(userID);
    ...
    public static  Userkeys keysGen(List<UserAuthorityKey> userAKeys,Userkeys userkeys){
...
//            Element temp=pairing.getGT().newOneElement().getImmutable();
            ...
            rjk.setFromBytes(userkeys.getUserAuthKeys().get(authorityID).getRjk());
